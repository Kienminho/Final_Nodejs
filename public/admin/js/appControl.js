...
$("#uploadForm").submit(function (e) {
    ...
    if (!$("#fileName").val() || !source || !image || category == "none" || !description) {
...
$("table").on("click", '#editDetail',function(e){
    ...
    $('#editModal').find("h4").html('Update app id &nbsp;<strong>' + id + '</strong>')
...
function saveChangeFile(){
  ...
  }else if((fileType == "pdf" && category != "B") ||
          (fileType == "apk" && category != "A" && category != "G")){
      $('#fileModal').find("#alert").text('This file cannot be put into that category!')
      $('#fileModal').find("#alert").fadeIn("fast");
  }else{
