...
let handelComment = (req, res) => {
  ...
  if (comment === "") {
    res.json({ status: 303, message: "Vui lòng nhập đánh giá của bạn." });
  }
  const newComment = {
    userName: req.session.name,
    idApp: idAppCurrent,
  ...
  Comment.create(newComment)
    .then((comment) => {
      console.log("done");
      res.json({ status: 200, message: "Cảm ơn bạn đã đánh giá" });
    })
    .catch((error) => {
      console.log(error);
    });
